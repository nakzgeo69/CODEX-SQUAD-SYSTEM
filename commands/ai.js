// ========== ai.js - PRECISE & ACCURATE VERSION ==========
...
module.exports = {
  ...
  description: 'Complete AI assistant with precise and accurate responses',
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      const finalPrompt = this.buildStrictPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed, subject);
      ...
      aiResponse = this.applyStrictFormat(aiResponse, prompt, subject);
  ...
  // ========== ENSURE COMPLETE RESPONSE - NO TRUNCATION ==========
  ensureComplete(text) {
    if (!text) return text;
    ...
    // Check if response has incomplete sentence
    ...
    // If response is too short and looks truncated, add a note
  ...
  // ========== APPLY STRICT FORMAT ==========
  applyStrictFormat(text, prompt, subject) {
    let formatted = text;
    ...
    // If problem solving and no "Answer:" prefix, add it
    if (this.isProblemSolvingRequest(prompt) || 
        subject === 'math' || subject === 'physics' || 
        subject === 'chemistry' || subject === 'statistics') {
      ...
      // Ensure "Answer:" prefix if missing
      if (!formatted.match(/^(Answer|Sagot|Solution):/i)) {
        const lines = formatted.split('\n');
        const firstLine = lines[0] || '';
        if (firstLine && !firstLine.match(/^(Step|Given|Formula)/i)) {
          formatted = 'Answer: ' + formatted;
        }
      }
    }

    // For non-problem solving, ensure "Answer:" prefix
    if (!this.isProblemSolvingRequest(prompt) && 
        subject !== 'math' && subject !== 'physics' && 
        subject !== 'chemistry' && subject !== 'statistics') {
      if (!formatted.match(/^(Answer|Sagot):/i)) {
        const lines = formatted.split('\n');
        const firstLine = lines[0] || '';
        if (firstLine && !firstLine.match(/^(Definition|Concept|Key|Main)/i)) {
          formatted = 'Answer: ' + formatted;
        }
      }
    }
  ...
  // ========== BUILD STRICT PROMPT ==========
  buildStrictPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed, subject) {
    let finalPrompt = '';

    const subjectInstructions = {
      'history': 'Provide a COMPLETE, ACCURATE, and THOROUGH historical response with key events and significance.',
      'social_science': 'Provide a COMPLETE, ACCURATE, and PRACTICAL response with key concepts and applications.',
      'science': 'Provide a COMPLETE, ACCURATE, and SCIENTIFICALLY-GROUNDED response with clear explanations.',
      'health': 'Provide a COMPLETE, RELIABLE, and PRACTICAL health response with key information.',
      'literature': 'Provide a COMPLETE, THOUGHTFUL, and WELL-REASONED literary analysis.',
      'business': 'Provide a COMPLETE, PRACTICAL, and REALISTIC business response with key insights.',
      'technology': 'Provide a COMPLETE, ACCURATE, and UP-TO-DATE technical response with clear explanations.',
      'math': 'Provide a COMPLETE STEP-BY-STEP solution with clear explanations and final answer.',
      'physics': 'Provide a COMPLETE STEP-BY-STEP solution with clear explanations and final answer.',
      'chemistry': 'Provide a COMPLETE STEP-BY-STEP solution with clear explanations and final answer.',
      'statistics': 'Provide a COMPLETE STEP-BY-STEP solution with clear explanations and final answer.',
      'general': 'Provide a COMPLETE, ACCURATE, and RELIABLE response with key information.'
    };

    const instruction = subjectInstructions[subject] || subjectInstructions.general;

    // ===== STRICT FORMAT RULES =====
    finalPrompt += 'STRICT FORMAT RULES:\n';
    finalPrompt += '1. Start directly with "Answer:" or "Sagot:"\n';
    finalPrompt += '2. NO introduction like "Let me", "I will", "Here is"\n';
    finalPrompt += '3. NO conclusion like "I hope this helps", "Please let me know"\n';
    finalPrompt += '4. Use plain text only. No markdown, no emojis.\n';
    finalPrompt += '5. Be COMPLETE. Do NOT use "..." to truncate.\n';
    finalPrompt += '6. For math/physics/chemistry/statistics: Show ALL steps and end with "Final Answer:"\n';
    finalPrompt += '7. For history/biography: Use complete sentences with key facts\n';
    finalPrompt += '8. For science/health: Use clear explanations with key points\n';
    finalPrompt += '9. For business/technology: Use practical insights with key details\n';
    finalPrompt += '10. DO NOT ask questions back - just provide the complete response\n\n';
    ...
    if (this.isProblemSolvingRequest(prompt) || 
      ...
      finalPrompt += 'Answer: [Direct answer if known]\n\n';
      finalPrompt += 'Step 1: [Description]\n';
      finalPrompt += '[Calculation]\n\n';
      finalPrompt += 'Step 2: [Description]\n';
      finalPrompt += '[Calculation]\n\n';
      finalPrompt += 'Step 3: [Description]\n';
      finalPrompt += '[Calculation]\n\n';
      finalPrompt += '(Continue steps as needed)\n\n';
      finalPrompt += 'Final Answer: [Result with units]\n\n';
    }

    if (wantsDetailed) {
      finalPrompt += 'Provide a COMPREHENSIVE and DETAILED response.\n';
    } else {
      finalPrompt += 'Provide a COMPLETE and CLEAR response.\n';
    }

    finalPrompt += '\nREMEMBER: Start with "Answer:". NO introduction. NO conclusion. NO "..." truncation.';
