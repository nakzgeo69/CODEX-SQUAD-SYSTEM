...
module.exports = {
  ...
  description: 'Complete AI assistant with auto-send all chunks',
  ...
  async sendAllChunks(senderId, text, token) {
    ...
    // If text is short enough, send as is
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      // Remove "Answer:" from subsequent chunks (only first chunk has it)
      if (!firstChunk) {
  ...
  applyStrictFormat(text, prompt, subject) {
    ...
    if (!formatted.match(/^(Answer|Sagot|Solution):/i)) {
      ...
      if (firstLine && !firstLine.match(/^(Step|Given|Formula|Definition|Key|Main)/i)) {
