...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      const isScience = subjectInfo.isScience;
      const isSocialScience = subjectInfo.isSocialScience;
      const isHumanities = subjectInfo.isHumanities;
      const isBusiness = subjectInfo.isBusiness;
      const isTechnology = subjectInfo.isTechnology;
      const isArts = subjectInfo.isArts;
      const isHealth = subjectInfo.isHealth;
      const isEducation = subjectInfo.isEducation;
      const isHistory = subjectInfo.isHistory;
      const isGeography = subjectInfo.isGeography;
      const isPhilosophy = subjectInfo.isPhilosophy;
      const isPsychology = subjectInfo.isPsychology;
      const isSociology = subjectInfo.isSociology;
      const isEconomics = subjectInfo.isEconomics;
      const isPoliticalScience = subjectInfo.isPoliticalScience;
      const isLaw = subjectInfo.isLaw;
      const isMedicine = subjectInfo.isMedicine;
      const isEngineering = subjectInfo.isEngineering;
      const isComputerScience = subjectInfo.isComputerScience;
      const isAgriculture = subjectInfo.isAgriculture;
      const isEnvironmentalScience = subjectInfo.isEnvironmentalScience;
      
      // ===== CHECK IF IT'S A SUBJECT-SPECIFIC REQUEST =====
      ...
      else if (isSubjectSpecific && confidenceScore > 0.5) {
  ...
  /**
   * Detects the subject/category of the question
   * Returns: { subject, isMath, isScience, isSocialScience, ... , confidence }
   */
  detectSubject(prompt) {
    if (!prompt) return { subject: 'general', confidence: 0, isMath: false };
    ...
    const subjects = {
      ...
      math: {
        keywords: [
          ...
          'statistics', 'probability', 'arithmetic', 'fraction', 'decimal',
          'percentage', 'quadratic', 'polynomial', 'matrix', 'vector',
          'derivative', 'integral', 'limit', 'sequence', 'series',
          'permutation', 'combination', 'binomial', 'logarithm', 'exponent',
          'mean', 'median', 'mode', 'variance', 'standard deviation',
          'frequency distribution', 'grouped data', 'ungrouped data',
          'hypothesis testing', 'regression', 'correlation',
          // Math problem patterns
          /[\d\s\+\-\*\/\(\)]+\s*=\s*[xX]/, // equations
          /\d+\s*[\+\-\*\/]\s*\d+/, // arithmetic
          /find [xX]/, /solve for [xX]/, /calculate/i, /compute/i
        ],
        weight: 3
      },
      ...
      socialScience: {
        keywords: [
          ...
          // Psychology
          'psychology', 'behavior', 'mental', 'cognition', 'emotion',
          'personality', 'perception', 'memory', 'learning',
          'development', 'motivation', 'attitude', 'mental health',
          ...
          'fiscal', 'monetary', 'banking', 'capital', 'labor',
          // Geography
          'geography', 'map', 'region', 'climate', 'terrain',
          'population', 'migration', 'urban', 'rural', 'land',
          'resource', 'environment', 'ecosystem', 'biome'
        ],
        weight: 2
      },
      ...
      humanities: {
        keywords: [
          ...
          'play', 'poetry', 'prose', 'narrative', 'character',
          'theme', 'symbolism', 'metaphor', 'allegory',
      ...
      technology: {
        keywords: [
          ...
          'neural network', 'algorithm', 'python', 'java', 'c++', 'javascript',
          'web development', 'app development', 'mobile app', 'api',
          'cloud computing', 'devops', 'agile', 'scrum', 'sprint',
      ...
      arts: {
        keywords: [
          ...
          'abstract', 'realism', 'impressionism', 'cubism',
      ...
      health: {
        keywords: [
          ...
          'epidemiology', 'public health', 'nutrition', 'exercise'
        ],
        weight: 2
      },
      ...
      education: {
        keywords: [
          ...
          'educational technology', 'instruction', 'methodology'
        ],
        weight: 2
      },
      ...
      engineering: {
        keywords: [
          'engineering', 'engineer', 'design', 'construct', 'build',
          'structure', 'machine', 'mechanical', 'electrical', 'civil',
          'structural', 'aerospace', 'material', 'manufacturing',
          'process', 'system', 'analysis', 'simulation', 'prototype'
        ],
        weight: 2
      },
      ...
      environmentalScience: {
        keywords: [
          ...
          'deforestation', 'biodiversity', 'ozone', 'global warming',
          'climate change', 'green', 'ecological', 'natural resources'
        ],
        weight: 2
      }
    };
      ...
      for (const keyword of data.keywords) {
        if (typeof keyword === 'string') {
          ...
          if (keyword.test(lower)) {
            matchCount++;
            score += data.weight * 2; // Regex patterns are stronger indicators
          }
        }
      }
      
      // Boost score if multiple keywords match
      if (matchCount >= 2) score += 5;
    ...
    return {
      ...
      isMath: bestSubject === 'math' || (bestScore > 0 && this.isMathProblem(prompt)),
      isScience: bestSubject === 'science' || bestSubject === 'biology' || bestSubject === 'chemistry' || bestSubject === 'physics',
      isSocialScience: bestSubject === 'socialScience' || bestSubject === 'sociology' || bestSubject === 'psychology' || bestSubject === 'politicalScience' || bestSubject === 'economics' || bestSubject === 'geography',
      isHumanities: bestSubject === 'humanities' || bestSubject === 'history' || bestSubject === 'philosophy' || bestSubject === 'literature',
      isBusiness: bestSubject === 'business',
      isTechnology: bestSubject === 'technology',
      isArts: bestSubject === 'arts',
      isHealth: bestSubject === 'health',
      isEducation: bestSubject === 'education',
      isHistory: bestSubject === 'history',
      isGeography: bestSubject === 'geography',
      isPhilosophy: bestSubject === 'philosophy',
      isPsychology: bestSubject === 'psychology',
      isSociology: bestSubject === 'sociology',
      isEconomics: bestSubject === 'economics',
      isPoliticalScience: bestSubject === 'politicalScience',
      isLaw: bestSubject === 'law',
      isMedicine: bestSubject === 'health' || bestSubject === 'medicine',
      isEngineering: bestSubject === 'engineering',
      isComputerScience: bestSubject === 'technology',
      isAgriculture: bestSubject === 'agriculture',
      isEnvironmentalScience: bestSubject === 'environmentalScience',
      scores: subjectScores,
      matchCount: Object.values(subjectScores).filter(s => s > 0).length
    };
  },
  ...
  buildSubjectPrompt(prompt, subject, language, wantsDetailed) {
    ...
    const subjectDescriptions = {
      ...
      socialScience: 'social science expert',
      humanities: 'humanities expert',
      ...
      sociology: 'sociology expert',
      economics: 'economics expert',
      politicalScience: 'political science expert',
      law: 'law expert',
      engineering: 'engineering expert',
      agriculture: 'agriculture expert',
      environmentalScience: 'environmental science expert'
    };
    
    const subjectName = subjectDescriptions[subject] || 'expert';
    ...
    if (isTagalog) {
      ...
      promptText += `- Tumugon bilang isang ${subjectName}\n`;
      ...
      promptText += `INSTRUKSYON:\n`;
      promptText += `- Tubag isip usa ka ${subjectName}\n`;
      ...
      promptText += `- Respond as a ${subjectName}\n`;
  ...
  // ===== BUILD REPLY PROMPT WITH SUBJECT CONTEXT =====
  ...
  // ===== MATH DETECTION (UPDATED) =====
  isMathProblem(prompt) {
    if (!prompt) return false;
    const lower = prompt.toLowerCase();
    
    // Strong math indicators (pure math)
    ...
    for (const pattern of strongMathPatterns) {
      if (pattern.test(lower)) {
        return true;
      }
    }
    
    // Check if it's clearly a math question (has math keywords AND asks for calculation)
    const mathKeywords = [
    ...
    if (hasMathKeyword && hasCalculationWord) {
      return true;
    }
  ...
  // ===== EXAMPLE REQUEST DETECTION (UPDATED) =====
  isExampleRequest(prompt) {
    if (!prompt) return false;
    const lower = prompt.toLowerCase();
    
    // Check if it's asking for examples in a non-math context
    ...
    // If it's asking for examples AND it's a math problem, return true
    if (this.isMathProblem(prompt)) {
      return true;
    }
    
    // Otherwise, it's a general example request (handled by subject detection)
    return false;
  },
